# Documentação do Purpura 💜
Este repositório contém a documentação do projeto **Purpura Ambiental**. Aqui você encontrará informações sobre as APIs, padrões de desenvolvimento, funções e delegações, entre outros tópicos relevantes para o projeto.

## Base

### Funções e Delegações
- [Funções](./funcoes/funcoes.md)
- [Delegações](./delegacoes/delegacoes.md)

### Padrões de desenvolvimento
- [Padrão de Banco](./padroes/padrao_banco.md)
- [Padrão de Commit](./padroes/padrao_commit.md)

### Fluxos de requisição das APIs
- [Fluxos Mobile](./api/fluxos.md)

## Informações Adicionais Técnicas

### Apis

- [Portas de API](./api/portas.md)
